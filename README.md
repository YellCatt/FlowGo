...
在「分组」中填写一个组名后，同一分组下的工作流在定时触发时**串行执行**——某次运行尚未结束时，同分组的下一次触发会被自动跳过（避免重复发邮件、重复写库等副作用）；**不同分组之间并行执行**。留空表示不分组，即每个工作流各自独立、互不阻塞（等价于之前的 per-workflow 互斥）。

> 注意：分组互斥仅作用于 **Cron 定时触发**；手动运行（`▶ 运行`）与 Webhook 触发不走调度器，仍可能并行。
